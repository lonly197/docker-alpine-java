...
LABEL \
    ...
    org.label-schema.description="Base and Clean Docker image with Oracle Java Runtime Time 8." \
...
RUN	set -x \
    ...
    && cd /tmp/ \
    && wget --no-check-certificate -c -q --header "Cookie: oraclelicense=accept-securebackup-cookie" http://download.oracle.com/otn-pub/java/jdk/${JAVA_VERSION}-b${JAVA_BUILD}/${JAVA_PATH}/jre-${JAVA_VERSION}-linux-x64.tar.gz \
